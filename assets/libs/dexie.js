    ...
    function Dexie(dbName, options) {
        ...
        function init() {
            ...
            db.on("versionchange", function(ev) {
                ...
                if (ev.newVersion > 0) console.warn('Another connection wants to upgrade database \'' + db.name + '\'. Closing db now to resume the upgrade.');
                else console.warn('Another connection wants to delete database \'' + db.name + '\'. Closing db now to resume the delete request.');
                db.close();
            ...
            db.on("blocked", function(ev) {
                if (!ev.newVersion || ev.newVersion < ev.oldVersion) console.warn('Dexie.delete(\'' + db.name + '\') was blocked');
                else console.warn('Upgrade \'' + db.name + '\' blocked by other connection holding version ' + ev.oldVersion / 10);
            });
        }
        ...
        this._tableFactory = function createTable(mode, tableSchema) {
            ...
            if (mode === READONLY) return new Table(tableSchema.name, tableSchema, Collection);
            else return new WriteableTable(tableSchema.name, tableSchema);
        };
                ...
                if (!hasDeleteHook) {
                    ...
                    tryCatch(function() {
                        for (var i = 0; i < len; ++i) {
                            ...
                            if (i === lastItem) req.onsuccess = hookedEventSuccessHandler(resolve);
            ...
            bulkPut: function(objects, keys$$1) {
                ...
                return this._idbstore(READWRITE, function(resolve, reject, idbstore) {
                    ...
                    var done = function(result) {
                        ...
                        else reject(new BulkError(_this.name + '.bulkPut(): ' + errorList.length + ' of ' + numObjs + ' operations failed', errorList));
                    };
                    var req,
                        errorList = [],
                        errorHandler,
                ...
                return this._idbstore(READWRITE, function(resolve, reject, idbstore, trans) {
                    ...
                    if (creatingHook !== nop) {
                        ...
                        if (effectiveKey == null && keyToUse != null) {
                            ...
                            if (idbstore.keyPath) setByKeyPath(obj, idbstore.keyPath, keyToUse);
                            else key = keyToUse;
                        }
                    }
                    try {
